...
This document specifies the algorithms underlying all security-critical and operationally significant behaviours of the Driflly system. Pseudocode uses a consistent notation: `//` for inline comments, `||` for byte-level concatenation, and function names in PascalCase. Time values are in seconds unless otherwise stated. All cryptographic comparisons use constant-time implementations where noted.
...
**Purpose:** Create a new ephemeral session, persist metadata, and return access credentials to the session creator.

**Bug fixed from v1.0:** `expiry_time = creation_time + 365 days` corrected to `creation_time + (duration_minutes * 60)`. The 365-day placeholder prevented the expiry scheduler from ever firing on normal sessions.
...
**Notes:**
- `expiry_time` governs when the chat timer elapses. Codes expire after 30 seconds. Links are single-use. These are independent expiry windows.
- No message content, encryption keys, or participant identifiers appear in the database at any stage.

---

## 2. Key Generation and Exchange

**Purpose:** Ensure both session participants hold the same AES-256-GCM key while minimising server-side key exposure.

**Improvement from v1.0:** The code join path now shows the explicit HTTP POST mechanism rather than the opaque `RetrieveKeyFromServer()` call. The mechanism is security-relevant because it determines exactly when and how the server observes the session key.
...
BEGIN
  IF join_method == 'creator' THEN
    // Creator generates key entirely client-side.
    // Key never leaves the browser at this stage.
  ...
  ELSE IF join_method == 'code' THEN
    // Server returns the key in the code redemption HTTP response.
  ...
  ELSE IF join_method == 'link' THEN
    // Key is embedded in the URL fragment (#key=...).
    // Fragments are never transmitted in HTTP requests.
    // The server never sees the key on this path.
    ...
    key_id = Base64Encode(raw_key_bytes)[0:8]
    StoreKeyInMemory(key_id, encryption_key)
    ClearURLFragment()   // Prevent key persisting in browser history
...
**Purpose:** Encrypt plaintext, attach replay protection fields, and deliver the secure envelope via WebSocket.

**Complexity:** O(n) where n is message length \u2014 dominated by AES-GCM encryption.
...
BEGIN
  iv         = crypto.getRandomValues(Uint8Array(12))  // 96-bit GCM nonce
  ...
  ELSE
    QueueMessageForRetry(secure_message)       // See Algorithm 12
...
**Notes:**
- The GCM `iv` embedded in `encrypted_payload` and the replay-protection `nonce` are distinct fields serving distinct purposes. The IV prevents GCM nonce reuse; the nonce enables server-side replay detection without the server holding the session key.
- The sequence counter increments AFTER confirmed send, not before, to prevent gaps from failed transmissions.

---

## 4. Message Reception and Validation

**Purpose:** Validate all security fields of an incoming message envelope before decryption, then deliver plaintext to the UI.

**Bug fixed from v1.0:** Sequence and nonce state are no longer committed inside the validation functions. State is committed only after ALL checks pass including HMAC, preventing a TOCTOU attack where a forged envelope could poison sequence state before the HMAC check rejected it.

**Complexity:** O(n) \u2014 validation gates are O(1) hash lookups; decryption is O(n).
...
BEGIN
  ...
  // Replay check 1: sequence monotonicity (read-only, no state commit yet)
  ...
  // Replay check 2: nonce uniqueness (read-only, no state commit yet)
...
**Notes:**
- Validation gates execute in the order shown. Schema and sequence checks precede HMAC to avoid unnecessary cryptographic work on malformed input.
- `ConstantTimeEqual` is mandatory. Variable-time comparison leaks byte-match information enabling a forgery oracle.
- AES-GCM's auth tag provides a second independent integrity check on the raw ciphertext. The HMAC covers the encoded envelope fields including sequence and nonce.

---

## 5. Replay Protection State Management

**Purpose:** Maintain server-side state enabling detection of replayed or forged message envelopes.

**Bug fixed from v1.0:** `ValidateSequence` and `ValidateNonce` are now read-only. State commit is handled by `UpdateLastSequence` and `StoreNonce`, called by `ReceiveAndValidateMessage` only after HMAC verification passes.
...
- `ValidateNonce`: O(k) per call for expiry cleanup; amortised O(1) under typical message rates
...
FUNCTION ValidateSequence(session_id, client_id, sequence):
  ...
  IF client_id NOT IN sequences[session_id] THEN
    // First message from this client \u2014 valid by definition
...
FUNCTION CleanupSessionReplayState(session_id):
  // Called by TerminateSession (Algorithm 9).
...
- Sequence state: O(P) where P <= MAX_PARTICIPANTS \u2014 at most 2 entries.
- Nonce state: O(R x NONCE_EXPIRY_SECONDS) where R is messages per second. At 40 msg/s the cache holds at most 12,000 entries.

---

## 6. Rate Limiting

**Purpose:** Prevent brute-force enumeration of six-digit access codes by bounding the redemption attempt rate per source IP address.

**Note:** Missing from the v1.0 draft. The security model referenced rate limiting throughout but provided no algorithm. Added here.
...
FUNCTION CheckRateLimit(ip_address):
  ...
  // Reset window if observation period has elapsed
...
**Notes:**
- Applies to `/session/code/{code}` only. Session creation and status polling are not application-layer rate-limited.
- The fixed-window algorithm admits a burst at window boundaries. A sliding-window implementation provides smoother enforcement at the cost of additional memory per IP.

---

## 7. Code Generation and Validation

**Purpose:** Produce a six-digit numeric code providing single-use, time-limited access to a session.

**Bug fixed from v1.0:** `ValidateCode` now marks the code as redeemed BEFORE returning success. The original sequence allowed two simultaneous requests to both read `redeemed=false` and both receive a valid response (TOCTOU race condition).
...
FUNCTION GenerateSixDigitCode(session_id, expires_at):
  ...
  // Guarantee uniqueness among currently active, unredeemed codes
  offset      = 0
  code_int    = base_code
  code_string = FormatSixDigits(code_int)   // Zero-padded: "007421"
...
FUNCTION InvalidateSessionCodes(session_id):
  // Called during session termination (Algorithm 9).
...
**Purpose:** Manage the pool of active connections per session, enforce participant limits, route validated messages, and detect stale connections via heartbeat.

**Bug fixed from v1.0:** `missed_pongs` is now explicitly initialised to 0 before the heartbeat loop. `MAX_RECONNECT_ATTEMPTS` correctly moved to the client-side reconnection function \u2014 it is not a server-side constant and had no effect in its previous location.
...
FUNCTION HandleNewConnection(websocket, session_id):
  ...
  IF CountConnections(session_id) == MAX_PARTICIPANTS THEN
    ...
    MarkChatStarted(session_id)   // SET chat_started_at = NOW()
  END IF

  StartHeartbeat(websocket)
  DeliverQueuedMessages(session_id, websocket)   // Algorithm 12
...
FUNCTION RouteMessage(session_id, message, sender):
  ...
  FOR EACH recipient IN recipients:
    ...
    IF NOT send_result.ok THEN
      QueueMessage(session_id, message)   // Algorithm 12
    END IF
  END FOR
END FUNCTION
...
FUNCTION ScheduleReconnectTimeout(session_id, timeout_seconds):
  ...
  IF CountConnections(session_id) == 1 THEN
    // Participant did not return within the grace period
    BroadcastToSession(session_id, {type: "participant_left_permanently"})
  END IF
  // If count == 2 the participant reconnected; no action required
END FUNCTION
...
**Purpose:** Automatically terminate sessions whose duration has elapsed and reclaim all associated in-memory and database resources.

**Schedule:** Background task, runs every 60 seconds.

**Complexity:** O(E x P) where E is expired sessions found and P <= MAX_PARTICIPANTS.
...
FUNCTION TerminateSession(session_id, reason):
  ...
  CleanupSessionReplayState(session_id)   // Algorithm 5
  InvalidateSessionCodes(session_id)      // Algorithm 7
END FUNCTION
```
...
**Purpose:** Reposition the chat input bar when the virtual keyboard appears or dismisses, keeping the message history and header visually stationary.

**Bug fixed from v1.0:** The original applied `paddingBottom` to the message container, causing layout reflow and unintended viewport shifts. The correct approach uses `position: fixed` on the input bar with `bottom: keyboardHeight` so only the input bar moves. The message container is entirely unaffected.
...
**Purpose:** Maintain a countdown display synchronised with the server's authoritative expiry timestamp, with colour-coded urgency cues and extension broadcasting.

**Bug fixed from v1.0:** The original decremented `time_left--` on each tick. JavaScript `setInterval` is imprecise \u2014 this accumulates meaningful drift on sessions longer than a few minutes. The corrected version derives `time_left` from `expiration_time - GetCurrentUnixSeconds()` on each tick, staying accurate regardless of interval imprecision.
...
FUNCTION InitialiseTimer(duration_seconds, server_expires_at):
  // Use the server's authoritative timestamp, not a local calculation
...
FUNCTION OnTick():
  // Derive from authoritative timestamp on every tick to prevent drift
...
FUNCTION ExtendSession(additional_minutes):
  ...
  // Broadcast so the partner's timer also updates
...
FUNCTION OnTimeUpdateReceived(message):
  // Called on the non-initiating participant's client
...
**Purpose:** Buffer outbound messages during WebSocket unavailability and deliver them upon reconnection.
...
FUNCTION DeliverQueuedMessages(session_id, websocket):
  ...
  WHILE NOT message_queue[session_id].isEmpty():
    ...
    ELSE
      // Connection failed mid-delivery \u2014 leave remaining messages queued
...
**Purpose:** Illustrate the complete interleaved execution of all algorithms from session creation through termination. Algorithm references appear in brackets.
...
CREATOR:
  1.  session = HTTP_POST /session/create {duration: 5}        [Alg. 1]
      -> {session_id, code, link, expires_at}

  2.  encryption_key, key_id = KeyExchange('creator', null)    [Alg. 2]

  3.  DisplayCodeAndLink(session.code, session.link)

  4.  BEGIN polling every 2 seconds:
...
JOINER:
  ...
  6.  rate_check = CheckRateLimit(joiner_ip)                   [Alg. 6]
      IF NOT rate_check.allowed THEN
        DisplayRetryError(rate_check.retry_after); RETURN
      END IF

  7.  result = ValidateCode(code, joiner_ip)                   [Alg. 7]
...
SERVER:
  ...
  9.  RETURN {session_id, encryption_key: key_export}
